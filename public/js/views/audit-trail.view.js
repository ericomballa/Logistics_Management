...
export class AuditTrailView extends BaseView {
  ...
  async loadAuditLogs(query = '', page = 1) {
    try {
      ...
      let response;
      try {
        response = await dataService.getAuditLogs(query, page, this.itemsPerPage);
      } catch (error) {
      ...
      const { data: auditLogs, total } = response;
      ...
      if (auditLogs.length === 0) {
        tableBody.innerHTML =
          '<tr><td colspan="6" style="text-align:center">Aucune activité enregistrée</td></tr>';
      } else {
        tableBody.innerHTML = auditLogs
          .map(
            (log) => `
                    <tr>
                        <td data-label="Date">${new Date(log.timestamp).toLocaleString('fr-FR')}</td>
                        <td data-label="Utilisateur">${log.performedBy || log.userId}</td>
                        <td data-label="Action"><span class="badge badge-${this.getActionColor(log.action)}">${log.action}</span></td>
                        <td data-label="Entité">${log.entityType}</td>
